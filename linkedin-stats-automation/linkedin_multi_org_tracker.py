...
Version corrigée avec gestion optimisée des quotas Google Sheets
"""
...
class GoogleSheetsExporter:
    ...
    def update_daily_stats_sheet(self, combined_stats, org_id):
        ...
        try:
            ...
            if len(dates_to_process) < 20:  # Afficher la liste si pas trop longue
                print(f"   📋 Dates: {sorted(list(dates_to_process))}")
            ...
            for date in dates_to_process:
                ...
                if date in existing_dates_dict:
                    # Mise à jour uniquement si c'est aujourd'hui ou hier
                    if date == today or date == yesterday:
                        row_index = existing_dates_dict[date]
                        updates_by_date[row_index] = row_data
                        print(f"   📝 Mise à jour prévue pour {date} (ligne {row_index})")
                else:
            ...
            # 2. Mettre à jour les lignes existantes (uniquement aujourd'hui et hier)
            if updates_by_date:
                print(f"   🔄 Mise à jour de {len(updates_by_date)} dates (aujourd'hui/hier)")
        ...
        for idx, org in enumerate(self.organizations, 1):
            ...
            try:
                ...
                if idx < total_orgs:  # Pas de pause après la dernière organisation
                    print(f"\n⏱️  Pause de 30 secondes avant la prochaine organisation...")
                    time.sleep(30)
...
def main():
    ...
    print("Version corrigée avec gestion optimisée des quotas")
    print("="*60)
    ...
    # Demander confirmation si plus de 3 organisations (réduction du seuil)
