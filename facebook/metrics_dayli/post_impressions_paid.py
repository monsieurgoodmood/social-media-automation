...
API_VERSION = "v21.0"
TEST_METRIC = "post_impressions_paid"  # Nouvelle métrique à tester
...
def fetch_single_metric(api_version, post_id, metric, start_date, end_date):
    ...
    if response.status_code == 200:
        ...
        if not data:
            print(f"⚠️ Aucun relevé quotidien pour la métrique '{metric}'. Valeurs par défaut (0) seront utilisées.")
        else:
...
# Vérification de la validité de la métrique
if test_metric_validity(API_VERSION, POST_ID, TEST_METRIC):
    ...
    print(f"La métrique '{TEST_METRIC}' n'est pas valide pour ce post.")
