...
def append_to_csv(data, filename='facebook_post_insights.csv'):
    ...
    # Charger les données actuelles si le fichier existe
...
def process_data():
    ...
    for post in posts:
        ...
        # Vérifiez que la clé 'created_time' existe dans le post, sinon utiliser une date par défaut
        created_time = pd.to_datetime(post.get('created_time', pd.Timestamp.now()), errors='coerce')
